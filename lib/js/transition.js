...
document.addEventListener("DOMContentLoaded", function() {
  ...
  planetLinks.forEach(link => {
    link.addEventListener("click", function(event) {
      ...
      setTimeout(() => {
        console.log("planet1", planet, typeof planet);

        sunVideo.src = isIOS()
          ? planet.includes("Earth")
            ? `assets/globs/${planet.toLowerCase()}.mov`
            : `assets/globs/${planet.toLowerCase()}.hevc.mp4`
          : `assets/globs/${planet.toLowerCase()}.webm`;
        sunVideo.style.opacity = "1";
      }, 300);

      setTimeout(() => {
        // First remove existing source elements
        console.log("planet2", planet);
        ...
        if (isIOS()) {
          if (planet.includes("Earth")) {
            source.src = `assets/globs/earth.mov`;
            source.type = "video/mp4; codecs=hevc";
          } else {
            source.src = `assets/globs/${planet.toLowerCase()}.hevc.mp4`;
            source.type = "video/mp4; codecs=hevc";
          }
        } else {
          source.src = `assets/globs/${planet.toLowerCase()}.webm`;
          source.type = "video/webm";
        }

        // Add the new source element
        sunVideo.appendChild(source);
